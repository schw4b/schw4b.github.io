...
draft: false
subtitle: "Data are overvalued, and statistics are undervalued. New tools and technologies will likely not revolutionize healthcare but may confuse us about which data and evidence are real and which are not. More data is not always better, and a data example with voodoo statistics is provided at the end."
author: "Simon Schwab"
date: "2024-07-09"
...
![*Science* magazine front cover from 11. February 2011.](science.2011.331.issue-6018.largecover.jpg){width=33%}
...
Data alone does not represent any real value, even if it is big data. Data can't speak for themselves. Data is pretty dumb. Think about your favorite restaurant and your favorite dish. Imagine being served only unprepared raw food. That's what data is basically without statistics; it's just the ingredients, the uncooked parts. Data on their own are inedible food.

One may object that more data will always lead to less uncertainty. This is, in principle, true. In reality, however, I imagine the many attempts data are explored, analyzed, and reanalyzed until the desired result is obtained or until the model fits the data. Then, this process only *appears* to reduce the uncertainty when looking at the p-values and standard errors. However, the process by which the data was analyzed has increased our uncertainty, but this is often unrecognized. It is a fallacy called the garden of forking paths.
...
![Garden of forking paths, Parque Juan Carlos I, Madrid, Spain. (Photo by Victor on Unsplash)](victor-_qXjdWm8YEo-unsplash.jpg){width=33%}
...
Statistical software once tried to create the illusion that a nice graphical interface could enable statistics at the push of a button. The exact opposite has happened with the *R programming language*. We moved away from pressing buttons to writing code, which is, as an attractive side effect, more straightforward to reproduce.
...
The main problem here is we didn't know that $x_1$, $x_2$, and $x_3$ were the relevant biomarkers. Using univariable screening, with $p \le$ 0.05 for variable selection, `r k` out of a total of `r q` variables were statistically significant; this matches up with the alpha level of 5%.

I tried to adjust the p-values to control the false discovery rate at 5% using the Benjamini & Hochberg procedure; however, only $x_1$ remained statistically significant. So, this only partially helped. I wanted more specificity, and I had to pay with sensitivity.

In the next step, maybe a little bit desperate, I tried to use a different threshold of $p \le$ 0.001.
...
With this step, we have left the realm of science facts and entered the realm of science fiction. The results above show voodoo statistics with statistically significant results for the variables that are pure noise (except $x_1$). The model now explained `r sprintf("%.0f%%", summary(fit)$r.squared*100)` of the variance (adj. $R^2$).

Surely, a high-impact journal would publish such statistically significant results?

## So what?

So what does this mean? I find the analysis of high-dimensional data is extremely difficult. Certainly, there are situations where the problem is not so extreme, but there may be situations where the issue is even more extreme.

You may object that the simulations I made could be made differently, yielding different results. Yes, but we often don't know how good the signal-to-noise ratio is. And completely in the dark, we must be very careful in the process of analyzing high-dimensional data, particularly when we use model selection. Univariable screening is well-known for providing flawed results, but it is still widely used today (or widely misused, I should say).

For further reading, see “How to Do Bad Biomarker Research” by Frank Harrell [@Harrell2022-gi].
